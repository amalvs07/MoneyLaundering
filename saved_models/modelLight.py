...
logging.warning("Instantiating LightGBM Model...")

modelLGM = lgb.LGBMClassifier(random_state=42, class_weight={0: 1, 1: 12})

logging.warning("Fitting LightGBM Model...")

modelLGM.fit(X_train, y_train)
y_pred_lgbm = modelLGM.predict(X_test)

logging.warning("Saving Model...")

model_path = "saved_models/modelLG.pkl"
pickle.dump(modelLGM, open(model_path, 'wb'))
...
model_metric = {
    ...
    "Confusion_matrix": confusion_matrix(y_test, y_pred_lgbm).tolist(),
...
filename = 'reports/lightperfo.csv'
